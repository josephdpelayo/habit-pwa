...
async function recordPaidStripeSession(session) {
  ...
  // Tras pagar llegan casi a la vez el webhook y confirm-checkout-session (el
  // regreso del socio a la app). Los dos pasan la consulta de arriba y el índice
  // único deja entrar solo a uno; el otro tiene que irse aquí. Antes seguía de
  // largo: notificaba y mandaba el push por segunda vez, y activateMembership
  // reescribía el perfil — devolviendo el crédito si el socio ya había reservado.
  if (inserted.duplicate) return { activated: true, recorded: false, duplicate: true };
